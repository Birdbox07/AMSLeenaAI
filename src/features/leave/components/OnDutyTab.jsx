...
import { useOnDutyQuery, useOnDutyMutations } from "../hooks/useOnDutyQuery";
import { validateOnDutyForm } from "../leave.onduty.validators";
import { useCurrentUser, useMyReportees } from "../../employees/hooks/useEmployees";
import { useHasRole } from "../../../shared/access/role.store";

export default function OnDutyTab() {
  const { data: requests } = useOnDutyQuery();
  const STORE = useOnDutyMutations();
  ...
  const submitApply = () => {
    const errors = validateOnDutyForm(form);
    ...
    const request = {
      id: `OD${Date.now()}`,
      requestNumber: `ODN${Date.now().toString().slice(-6)}`,
    ...
    STORE.add(request);
    toast.success(`On-Duty request ${request.requestNumber} submitted`);
    setShowApply(false);
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <Btn variant="primary" size="sm" onClick={() => setShowApply(true)}>
          <Plus size={14} /> Request On-Duty
        </Btn>
      </div>

      <DataTable
        title={scope === "team" && isManager ? "My Reportees' On-Duty Requests" : "My On-Duty Requests"}
        columns={cols}
        data={filtered}
      ...
      <Modal open={showApply} onClose={() => setShowApply(false)} title="Request On-Duty"
        footer={<>
        ...
        <FormField label="Reason">
          <textarea rows={3} value={form.reason} onChange={e => setForm(f => ({ ...f, reason: e.target.value }))}
            placeholder="Reason for on-duty request..." className={inputCls + " resize-none"} />
        </FormField>
      </Modal>
    </div>
  );
