...
import {
  CheckCircle, Clock, Calendar, FileText, Building2,
  Tag, Mail, Phone, User, MapPin, LogOut, Headphones,
} from "lucide-react";
import { cn } from "../../shared/utils/cn";
import { StatusBadge } from "../../shared/components/StatusBadge";
import { useLeaveQuery } from "../leave/hooks/useLeaveQuery";
import { getLeaveBalance, LEAVE_TYPE_BY_EMP_DATE } from "../leave/leave.mock";
...
export default function DashboardPage({ onNavigate }) {
  ...
  const monthStats = useMemo(() => {
    let present = 0, earlyExit = 0, late = 0, absent = 0, onDuty = 0, forgotToSwipe = 0;
    currentMonthAttendance.forEach(r => {
      const isHoliday = HOLIDAY_DATA.some(h => h.date === r.date);
    ...
    return { present, earlyExit, late, absent, onDuty, forgotToSwipe };
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        <div className={cn("lg:col-span-3 bg-card rounded-lg border border-border shadow-sm p-5", "hover-lift", "animate-fade-in-up")} style={{ animationDelay: "240ms" }}>
          <h3 className="font-semibold text-sm flex items-center gap-2 mb-4"><Clock size={16} className="text-primary"/> My Attendance – Current Month</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 mb-3">
            {[
              ...
              { label:"Early Exit", value:monthStats.earlyExit, color:"bg-teal-100 text-teal-700 dark:bg-teal-900/30 dark:text-teal-300" },
              ...
              { label:"On Duty", value:monthStats.onDuty, color:"bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300" },
              { label:"Forgot to Swipe", value:monthStats.forgotToSwipe, color:"bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300" },
            ].map(s => (
        ...
        <div className="lg:col-span-1 flex flex-col gap-4">
          <div className={cn("bg-card rounded-lg border border-border shadow-sm p-5", "hover-lift", "animate-fade-in-up")} style={{ animationDelay: "300ms" }}>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="text-left text-muted-foreground border-b border-border">
                  <th className="pt-1.5 pr-2 pb-1.5 pl-0 font-medium">Type</th>
